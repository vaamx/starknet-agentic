...
exports.default = AgentLeaderboard;
const RANK_STYLES = {
...
function AgentLeaderboard({ entries, selectedAgent, onSelectAgent, }) {
    return (<div className="neo-card overflow-hidden">
      {/* Header */}
      ...
      <div>
        {entries.map((entry, i) => {
            ...
            return (<button key={entry.agent} onClick={() => onSelectAgent?.(entry.agent)} className={`w-full flex items-center px-4 py-2.5 text-left transition-all duration-100 animate-enter border-b border-gray-100 last:border-0 ${isSelected
                    ? "bg-neo-blue/5 border-l-4 border-l-neo-blue pl-3"
                    : "hover:bg-gray-50 border-l-4 border-l-transparent pl-3"}`} style={{ animationDelay: `${i * 50}ms` }}>
              {/* Rank */}
              <span className={`w-5 h-5 flex items-center justify-center text-[10px] font-black border border-black ${RANK_STYLES[entry.rank] ?? "bg-gray-100 text-gray-500"}`}>
                {entry.rank}
              </span>
              ...
              <div className="flex-1 min-w-0 ml-2.5">
                ...
                {entry.identity && (<p className="text-[9px] text-gray-400 mt-0.5 truncate">
              ...
              {/* Grade */}
              <span className={`w-5 h-5 flex items-center justify-center text-[9px] font-black border border-black ml-2 ${grade.bg} ${grade.text}`}>
                {grade.label}
              </span>
            </button>);
        })}
      ...
      {entries.length === 0 && (<div className="px-4 py-10 text-center">
