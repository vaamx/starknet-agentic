          ...
          {loading ? (
            ...
            <div className="space-y-5">
              ...
              {detail.preferredSources && detail.preferredSources.length > 0 && (
                <div>
                  <h4 className="mb-1 text-[10px] font-medium uppercase tracking-wider text-muted">
                    Data Sources
                  </h4>
                  <div className="flex flex-wrap gap-1">
                    {detail.preferredSources.map((src) => (
                      <span
                        key={src}
                        className="rounded-full bg-white/[0.06] px-2 py-0.5 text-[10px] text-white/60"
                      >
                        {src}
                      </span>
                    ))}
                ...
                <div className="flex gap-2">
                  {detail.status === "running" && (
                    <button
                      ...
                      className="neo-btn flex-1 text-xs text-yellow-400 disabled:opacity-50"
                    >
                      Pause
                    </button>
                  )}
                  {detail.status === "paused" && (
                    <button
                      ...
                      disabled={controlLoading}
                      className="neo-btn flex-1 text-xs text-green-400 disabled:opacity-50"
                    >
                      Resume
                    </button>
                  )}
                  {detail.status !== "stopped" && (
                    <button
                      ...
                      disabled={controlLoading}
                      className="neo-btn flex-1 text-xs text-red-400 disabled:opacity-50"
                    >
                      Stop
                    </button>
                  )}
