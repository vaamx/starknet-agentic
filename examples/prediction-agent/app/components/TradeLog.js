    ...
    return (<div className="neo-card overflow-hidden">
      ...
      <div className="overflow-hidden border-b border-gray-100">
        <div className="ticker-tape flex items-center gap-6 px-4 py-1.5 whitespace-nowrap">
          {[...DEMO_TRADES, ...DEMO_TRADES].map((trade, i) => (<span key={i} className="flex items-center gap-1.5 text-[10px]">
              <span className="text-gray-400 font-mono">{trade.time}</span>
              <span className="font-bold">{trade.user.slice(0, 8)}</span>
              <span className={`font-black ${trade.outcome === "YES" ? "text-neo-green" : "text-neo-pink"}`}>
                {trade.outcome}
              </span>
      ...
      <div className="divide-y divide-gray-100">
        {DEMO_TRADES.slice(0, 6).map((trade, i) => (<div key={i} className="flex items-center gap-2 px-4 py-2 text-xs hover:bg-gray-50/50 transition-colors">
            <span className="font-mono text-[10px] text-gray-300 w-8 shrink-0 tabular-nums">
              {trade.time}
            </span>
            <div className="flex items-center gap-1.5 w-24 shrink-0">
              {trade.isAgent && (<span className="w-3.5 h-3.5 bg-neo-purple text-white text-[7px] font-black flex items-center justify-center border border-black">
                  AI
                </span>)}
            ...
            <span className={`font-heading font-bold text-[11px] w-8 ${trade.outcome === "YES" ? "text-neo-green" : "text-neo-pink"}`}>
              {trade.outcome}
            </span>
