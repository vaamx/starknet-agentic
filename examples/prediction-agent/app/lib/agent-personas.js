...
exports.getPersonaIds = getPersonaIds;
exports.simulatePersonaForecast = simulatePersonaForecast;
...
function getPersona(id) {
    return exports.AGENT_PERSONAS.find((p) => p.id === id);
}
/** Get all persona IDs. */
function getPersonaIds() {
    return exports.AGENT_PERSONAS.map((p) => p.id);
}
/**
 ...
 */
function simulatePersonaForecast(persona, baseMarketProb, question) {
    ...
    const strength = Math.abs(probability - 0.5) > 0.3
        ? "strongly"
...
As a ${persona.agentType}, I approach this through my ${persona.id === "alpha"
...
- My ${persona.agentType} methodology gives weight to ${persona.confidence > 0.8
