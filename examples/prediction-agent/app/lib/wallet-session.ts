...
function buildSessionPayload(walletAddress: string): WalletSessionPayload {
...
export function issueWalletSessionToken(walletAddress: string): {
  token: string;
  ...
  const payload = buildSessionPayload(normalized);
  return {
...
export function requireWalletSession(request: Request):
  | { ok: true; walletAddress: string; expiresAt: number }
