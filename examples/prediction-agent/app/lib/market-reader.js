...
exports.DEMO_QUESTIONS = void 0;
exports.getMarkets = getMarkets;
exports.getMarketState = getMarketState;
exports.getAgentPredictions = getAgentPredictions;
exports.getWeightedProbability = getWeightedProbability;
exports.getDemoLeaderboard = getDemoLeaderboard;
...
async function getMarkets() {
    if (config_1.config.MARKET_FACTORY_ADDRESS === "0x0")
        return getDemoMarkets();
    const factory = new starknet_1.Contract(FACTORY_ABI, config_1.config.MARKET_FACTORY_ADDRESS, provider);
...
async function getMarketState(id, address) {
    const market = new starknet_1.Contract(MARKET_ABI, address, provider);
    ...
    return {
        ...
        impliedProbYes: (0, accuracy_1.fromScaled)(BigInt(probs[1]?.[1]?.toString() ?? "500000000000000000")),
        impliedProbNo: (0, accuracy_1.fromScaled)(BigInt(probs[0]?.[1]?.toString() ?? "500000000000000000")),
    };
}
/** Get agent predictions for a market. */
async function getAgentPredictions(marketId) {
    if (config_1.config.ACCURACY_TRACKER_ADDRESS === "0x0")
        return getDemoPredictions(marketId);
    const tracker = new starknet_1.Contract(ACCURACY_ABI, config_1.config.ACCURACY_TRACKER_ADDRESS, provider);
    ...
    for (let i = 0; i < count; i++) {
        ...
        predictions.push({
            agent,
            marketId,
            predictedProb: (0, accuracy_1.fromScaled)(BigInt(prediction.toString())),
            brierScore: (0, accuracy_1.averageBrier)(BigInt(cumulative.toString()), BigInt(predCount.toString())),
...
async function getWeightedProbability(marketId) {
    if (config_1.config.ACCURACY_TRACKER_ADDRESS === "0x0")
        return 0.5;
    const tracker = new starknet_1.Contract(ACCURACY_ABI, config_1.config.ACCURACY_TRACKER_ADDRESS, provider);
    const result = await tracker.get_weighted_probability(marketId);
    return (0, accuracy_1.fromScaled)(BigInt(result.toString()));
}
...
function getDemoPredictions(marketId) {
    const agents = [
