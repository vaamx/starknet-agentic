...
exports.fetchPolymarketData = fetchPolymarketData;
const GAMMA_API = "https://gamma-api.polymarket.com";
/**
 * Search Polymarket for markets related to the question.
 */
async function fetchPolymarketData(question) {
    ...
    try {
        ...
        if (Array.isArray(markets) && markets.length > 0) {
            const data = markets.slice(0, 5).map((m) => ({
...
function formatOdds(outcomePrices) {
    if (!outcomePrices)
        return "N/A";
    try {
...
function parseOdds(outcomePrices) {
    try {
        const prices = typeof outcomePrices === "string"
            ? JSON.parse(outcomePrices)
            : outcomePrices;
...
function getDemoPolymarketData(question, keywords) {
