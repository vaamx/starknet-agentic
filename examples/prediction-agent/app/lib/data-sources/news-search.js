...
async function fetchNewsData(question) {
...
function detectCategory(question) {
    ...
    if (/\b(eth|btc|strk|crypto|defi|blockchain|bitcoin|ethereum|starknet|token|nft)\b/.test(q))
        return "crypto";
    if (/\b(president|election|congress|senate|vote|bill|law|regulation|government|policy)\b/.test(q))
        return "politics";
    if (/\b(super bowl|championship|nba|nfl|world cup|soccer|football|basketball|baseball|win|game)\b/.test(q))
        return "sports";
    if (/\b(apple|google|microsoft|ai|launch|release|phone|chip|software)\b/.test(q))
        return "tech";
    return "general";
}
function getDemoNewsData(question, keywords) {
    const category = detectCategory(question);
    const templates = NEWS_TEMPLATES[category];
    const data = templates.slice(0, 5).map((t) => ({
        label: t.title,
        value: t.snippet,
        url: t.url,
    }));
