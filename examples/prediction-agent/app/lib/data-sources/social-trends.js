...
exports.fetchSocialTrends = fetchSocialTrends;
async function fetchSocialTrends(question) {
...
function analyzeSentiment(question) {
    ...
    if (net > 1)
        return { label: "Strongly Bullish", score: 0.8, volume: "High" };
    if (net > 0)
        return { label: "Mildly Bullish", score: 0.6, volume: "Moderate" };
    if (net < -1)
        return { label: "Strongly Bearish", score: 0.2, volume: "High" };
    if (net < 0)
        return { label: "Mildly Bearish", score: 0.4, volume: "Moderate" };
    return { label: "Neutral", score: 0.5, volume: "Normal" };
}
function computeTrendScore(question) {
    const q = question.toLowerCase();
    let score = 40; // Base score
    // High-interest topics boost trending
    if (/eth|btc|bitcoin|ethereum/i.test(q))
        score += 25;
    if (/ai|artificial intelligence/i.test(q))
        score += 20;
    if (/super bowl|election|president/i.test(q))
        score += 30;
    if (/apple|google|openai/i.test(q))
        score += 15;
    if (/strk|starknet/i.test(q))
        score += 10;
    // Time-sensitive questions trend higher
    if (/this month|this week|today/i.test(q))
        score += 15;
    if (/2026/i.test(q))
        score += 5;
