 ...
 */
async function fetchCryptoPrices(question) {
...
function detectCryptoTokens(question) {
    ...
    if (priceMatch && found.size === 0) {
        ...
        if (/eth/i.test(question))
            found.add("ethereum");
        if (/btc|bitcoin/i.test(question))
            found.add("bitcoin");
        if (/strk|starknet/i.test(question))
            found.add("starknet");
    }
...
function getDemoCryptoPrices(tokens) {
    const demoPrices = {
