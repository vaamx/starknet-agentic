...
async function GET() {
    ...
    const stream = new ReadableStream({
        start(controller) {
            ...
            const recent = agent_loop_1.agentLoop.getActionLog(10);
            for (const action of recent) {
                controller.enqueue(encoder.encode(`data: ${JSON.stringify({ eventType: "action", ...action })}\n\n`));
            }
            // Subscribe to new actions
            const unsubscribe = agent_loop_1.agentLoop.subscribe((action) => {
                try {
                    controller.enqueue(encoder.encode(`data: ${JSON.stringify({ eventType: "action", ...action })}\n\n`));
                }
                catch {
