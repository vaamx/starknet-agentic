...
describe("parseArgs", () => {
    ...
    it("throws on missing --recipient value", () => {
        expect(() => parseArgs(["--recipient"])).toThrow("Missing value for --recipient");
    });
...
describe("validators", () => {
    it("validates private key format", () => {
        ...
        expect(() => assertPrivateKeyFormat("abc")).toThrow("Invalid PRIVATE_KEY format");
    });
    it("validates recipient address format", () => {
        ...
        expect(() => assertRecipientAddressFormat("0x" + "a".repeat(65))).toThrow("Invalid recipient address format");
        expect(() => assertRecipientAddressFormat("123abc")).toThrow("Invalid recipient address format");
    });
    it("rejects non-positive amounts", () => {
        expect(() => assertPositiveAmount("1")).not.toThrow();
        expect(() => assertPositiveAmount("0")).toThrow("Amount must be a positive number.");
        expect(() => assertPositiveAmount("-1")).toThrow("Amount must be a positive number.");
        expect(() => assertPositiveAmount("nope")).toThrow("Amount must be a positive number.");
    });
});
describe("sanitizeErrorForLog", () => {
    ...
    it("redacts literal env-var secrets set in process.env", () => {
        ...
        try {
            const sanitized = sanitizeErrorForLog(new Error("Request failed with plain-private-key-value and plain-paymaster-secret"));
