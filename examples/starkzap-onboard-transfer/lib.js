 ...
 */
export function assertPrivateKeyFormat(privateKey) {
    if (!PRIVATE_KEY_PATTERN.test(privateKey)) {
        throw new Error("Invalid PRIVATE_KEY format. Expected 0x-prefixed 64-hex string (example: 0x" +
            "a".repeat(64) +
            ").");
    }
}
 ...
 */
export function assertRecipientAddressFormat(recipientAddress) {
    if (!HEX_ADDRESS_PATTERN.test(recipientAddress)) {
        throw new Error("Invalid recipient address format. Expected 0x-prefixed 1-64 hex chars.");
    }
}
/**
...
export function sanitizeErrorForLog(err) {
    ...
    sanitized = sanitized.replace(/\b(PRIVATE_KEY|AVNU_PAYMASTER_API_KEY)\s*[:=]\s*[^\s,;]+/gi, "$1=[redacted]");
    for (const secret of [
