...
import { assertPositiveAmount, assertPrivateKeyFormat, assertRecipientAddressFormat, parseArgs, sanitizeErrorForLog, } from "./lib";
...
function logEvidence(doLog, data) {
    if (!doLog)
        return;
...
function assertWaitable(value) {
    if (typeof value !== "object" || value === null) {
        throw new Error("transfer response is missing wait()");
    }
    const maybeWait = value.wait;
    if (typeof maybeWait !== "function") {
        throw new Error("transfer response is missing wait()");
    }
}
async function main() {
    const { recipient, amount, sponsored, addressOnly, evidence } = parseArgs(process.argv.slice(2));
    const privateKey = process.env.PRIVATE_KEY?.trim();
    if (!privateKey) {
        console.error("Missing PRIVATE_KEY. Generate one: PRIVATE_KEY=0x$(openssl rand -hex 32)\n" +
    ...
    if (sponsored && !addressOnly && !paymasterApiKey) {
        console.error("Sponsored mode requires AVNU_PAYMASTER_API_KEY. Get one at https://portal.avnu.fi/");
        process.exit(1);
    }
    const recipientAddress = recipient || process.env.RECIPIENT_ADDRESS?.trim();
    if (!addressOnly && !recipientAddress) {
        console.error("Provide --recipient 0x... or set RECIPIENT_ADDRESS in .env");
        process.exit(1);
    }
    if (!addressOnly && recipientAddress) {
        try {
    ...
    if (evidence)
        console.log("Evidence: logging to", EVIDENCE_FILE);
    console.log("");
    const sdk = new StarkSDK(sponsored && paymasterApiKey
        ? {
            network: "sepolia",
            rpcUrl,
            paymaster: {
    ...
    const transferRecipient = recipientAddress;
    ...
    try {
        ...
        console.error("Invalid transfer amount. Provide a positive numeric value (e.g. 1 or 0.5).");
        logEvidence(evidence, { step: "invalid_amount", error: true });
        process.exit(1);
    }
    if (balance.lt(transferAmount)) {
        console.error(`Insufficient balance. Need ${amount} STRK. Get test tokens: https://starknet-faucet.vercel.app/`);
        logEvidence(evidence, { step: "insufficient_balance", error: true });
        process.exit(1);
    }
    console.log("[4/4] Sending", amount, "STRK to", transferRecipient, "...");
    const tx = await wallet.transfer(STRK, [{ to: fromAddress(transferRecipient), amount: transferAmount }], sponsored ? { feeMode: "sponsored" } : undefined);
    assertWaitable(tx);
    const txHash = getOptionalStringProperty(tx, "transactionHash") ??
        getOptionalStringProperty(tx, "transaction_hash");
    const explorerUrl = getOptionalStringProperty(tx, "explorerUrl") ??
