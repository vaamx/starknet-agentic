...
async function testDeployAccountParsesFactoryEvent() {
    const factoryAddress = "0x358301e1c530a6100ae2391e43b2dd4dd0593156e59adab7501ff6f4fe8720e";
    ...
    try {
        ...
        assert.equal(derivedFromReturnedPrivateKey.toLowerCase(), result.publicKey.toLowerCase());
...
async function testDeployAccountGasfreeUsesPaymasterPath() {
    let paymasterDetails;
    const mockDeployerAccount = {
        execute: async () => {
            throw new Error("execute should not be called in gasfree test");
        },
        executePaymasterTransaction: async (calls, details) => {
    ...
    const mockProvider = {
        waitForTransaction: async () => ({
            events: [
                {
                    from_address: "0x358301e1c530a6100ae2391e43b2dd4dd0593156e59adab7501ff6f4fe8720e",
                    data: ["0xacc", "0xpub", "0x2", "0x0", "0xreg"],
                },
            ],
        }),
...
async function testFirstActionBalanceReadOnlyFlow() {
    ...
    const result = await firstAction({
        provider: mockProvider,
        accountAddress: "0x6c876f3f05e44fbe836a577c32c05640e4e3c4745c6cdac35c2b64253370071",
        privateKey: "0x1",
        network: "sepolia",
        verifyTx: false,
    });
