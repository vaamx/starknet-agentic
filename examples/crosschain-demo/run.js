...
import { Account, CallData, ETransactionVersion, PaymasterRpc, byteArray, cairo, } from "starknet";
...
export function extractMintedTokenId(receipt, iface) {
...
export function parseFundingProvider(value) {
...
export function parseMinStarknetDeployerBalanceWei(value) {
    return parseNonNegativeWei(value || "5000000000000000", "MIN_STARKNET_DEPLOYER_BALANCE_WEI");
}
export function parseNonNegativeWei(raw, varName) {
    let parsed;
    try {
        parsed = BigInt(raw);
    }
...
export function parsePositiveIntEnv(value, varName, defaultValue) {
...
async function updateStarknetUri(args) {
    const paymasterUrl = args.paymasterUrl ||
...
async function main() {
    ...
    if (!evmConfig) {
        throw new Error(`Unknown EVM network "${args.evmNetwork}". Available: ${Object.keys(EVM_NETWORKS).join(", ")}`);
    }
    ...
    const fundingPollIntervalMs = parsePositiveIntEnv(process.env.FUNDING_POLL_INTERVAL_MS, "FUNDING_POLL_INTERVAL_MS", 5000);
    ...
    if (Number(evmNetwork.chainId) !== evmConfig.chainId) {
        throw new Error(`EVM chain mismatch: expected ${evmConfig.chainId}, got ${evmNetwork.chainId.toString()}`);
    }
    ...
    if (evmBalanceWei < minEvmBalanceWei) {
        throw new Error(`Insufficient EVM gas balance for ${evmWallet.address}: ` +
    ...
    console.log(`  Funding status: ${fundingStage.funding.status} (provider=${fundingStage.funding.provider}, deployer_balance=${formatEthWei(fundingStage.balanceWei)} ETH)`);
    // ---------- EVM register ----------
    console.log("[4/7] Registering EVM identity...");
    const predictedEvmAgentId = await evmIdentity.register.staticCall(PLACEHOLDER_URI);
    ...
    const sharedUri = args.sharedUri ||
        createSharedUri({
            name: args.name,
    ...
    if (starknetDeploy.agentId !== predictedStarknetAgentId) {
        throw new Error(`Predicted Starknet agent_id ${predictedStarknetAgentId} but deployed ${starknetDeploy.agentId}. ` +
    ...
    console.log(`Starknet tx (deploy): ${starknetPreflight.networkConfig.explorer}/tx/${starknetDeploy.deployTxHash}`);
}
