...
export async function fundDeployer(args) {
    ...
    if (!alreadyFunded && args.providerSelection === "skipped") {
        throw new Error("FUNDING_PROVIDER=skipped requires deployer balance >= MIN_STARKNET_DEPLOYER_BALANCE_WEI");
    }
    if (!alreadyFunded && args.providerSelection === "auto") {
        if (!args.config.l1RpcUrl || !args.config.l1PrivateKey) {
            throw new Error("Deployer balance is below MIN_STARKNET_DEPLOYER_BALANCE_WEI and no real funding provider is configured. " +
                "Set FUNDING_PROVIDER=mock for dry-run testing, or configure L1_RPC_URL + L1_PRIVATE_KEY for StarkGate funding.");
        }
    }
    const selected = alreadyFunded || args.providerSelection === "skipped"
        ? resolver("skipped")
    ...
    const funding = await selected.fund({
        ...
        readTargetBalanceWei: () => readTokenBalanceWei({
            provider: args.provider,
            network: args.network,
            accountAddress: args.deployerAddress,
            token: "ETH",
