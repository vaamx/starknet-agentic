...
function toU256Calldata(value) {
    const low = value & ((1n << 128n) - 1n);
    const high = value >> 128n;
    return [num.toHex(low), num.toHex(high)];
}
function tokenAddressFromSymbol(symbolOrAddress) {
...
function pubKeyFromPriv(privHex) {
    return num.toHex(ec.starkCurve.getStarkKey(privHex));
}
...
function parseToolTextJson(toolResponse) {
    const text = toolResponse?.content?.find?.((c) => c?.type === "text")?.text;
    if (typeof text !== "string")
        return { raw: toolResponse };
    try {
...
class McpSidecar {
    ...
    async connect() {
        ...
        const client = new Client({ name: `full-stack-swarm-${this.label}`, version: "0.1.0" }, { capabilities: {} });
...
function startSisna(args) {
    const defaultKeyId = args.signerProvider === "local"
    ...
    const env = {
        ...
        KEYRING_AUTH_CLIENTS_JSON: JSON.stringify(Object.entries(args.allowedKeyIdsByClientId).map(([clientId, allowedKeyIds]) => ({
            clientId,
    ...
    if (args.signerProvider === "local") {
        env.KEYRING_SIGNING_KEYS_JSON = JSON.stringify(Object.entries(args.signingKeysById).map(([keyId, privateKey]) => ({ keyId, privateKey })));
    }
    else {
...
function declareSessionAccountIfRequested(args) {
    ...
    const casmPath = path.join(targetDir, "session_account_SessionAccount.compiled_contract_class.json");
...
async function main() {
    ...
    const paymasterFeeMode = (envString("AVNU_PAYMASTER_FEE_MODE", "default") || "default");
    ...
    const sisnaSignerProvider = sisnaSignerProviderRaw;
    ...
    if (fundBeforeRun) {
        ...
        await Promise.all(state.agents.map((agent) => (async () => {
    ...
    const ownerResults = await Promise.all(state.agents.map((agent) => (async () => {
        await sem.acquire();
        ...
        try {
            ...
            if (!agent.agentId) {
                ...
                const reg = parseToolTextJson(await sidecar.callTool("starknet_register_agent", { token_uri: tokenUri, gasfree: gasfreeOwner }));
    ...
    if (startSisnaFlag) {
        if (!sisnaDir)
            throw new Error("START_SISNA=1 requires SISNA_DIR to be set");
        const signingKeysById = {};
        const allowedKeyIdsByClientId = {};
        for (const agent of state.agents) {
            allowedKeyIdsByClientId[`mcp-${agent.sessionKeyId}`] = [agent.sessionKeyId];
            if (sisnaSignerProvider === "local") {
                if (!agent.sessionPrivateKey)
                    continue;
        ...
        if (sisnaSignerProvider === "dfns") {
            if (!dfnsSignerUrl || !dfnsAuthToken || !dfnsUserActionSignature) {
                throw new Error("DFNS mode requires KEYRING_DFNS_SIGNER_URL, KEYRING_DFNS_AUTH_TOKEN, KEYRING_DFNS_USER_ACTION_SIGNATURE");
            }
            for (const agent of state.agents) {
    ...
    const tradeResults = await Promise.all(state.agents.map((agent) => (async () => {
        ...
        try {
            ...
            const balances = parseToolTextJson(await sidecar.callTool("starknet_get_balances", { tokens: balanceTokens }));
            const quote = parseToolTextJson(await sidecar.callTool("starknet_get_quote", { sellToken, buyToken, amount }));
            const swap = parseToolTextJson(await sidecar.callTool("starknet_swap", {
                sellToken,
                buyToken,
                amount,
                slippage,
    ...
    const ok = ownerResults.every((r) => r.ok) &&
        tradeResults.every((r) => r.ok);
    const report = {
        ok,
