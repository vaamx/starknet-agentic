 ...
 */
async function promptSetupMode(platform) {
 ...
 */
async function promptSkills(preselect = true) {
    const response = await prompts({
        type: "multiselect",
        name: "skills",
 ...
 */
async function promptConfigScope(platformName) {
    const response = await prompts({
        type: "select",
        name: "scope",
 ...
 */
function generateMcpConfig(network, secretsEnvPath) {
 ...
 */
function generateEnvTemplate(network) {
    const rpcUrl = RPC_URLS[network];
 ...
 */
function generateClaudeMd(selectedSkills, network) {
 ...
 */
function generateSkillsCommands(selectedSkills) {
    return selectedSkills.map((skill) => `npx skills add keep-starknet-strange/starknet-agentic/skills/${skill}`);
}
/**
 * Fetch directory contents from GitHub API
 */
async function fetchGitHubDirectory(skillId) {
    ...
    try {
        ...
        console.log(pc.yellow(`  Warning: Failed to fetch ${skillId}: ${error.message}`));
        return null;
    }
}
/**
 ...
 */
async function downloadSkillDirectory(skillId, targetDir, subPath = "") {
 ...
 */
async function fetchSkillMdOnly(skillId) {
 ...
 */
async function installSkills(selectedSkills, skillsPath, silent = false) {
    const installed = [];
    const failed = [];
    if (!silent) {
        console.log(pc.cyan("\nInstalling skills..."));
 ...
 * Write files to disk
 */
function writeFiles(files, silent = false) {
 ...
 */
export async function openclawWizard(platform, skipPrompts = false, defaultNetwork = "sepolia", jsonOutput = false, customSkills, _configScope = "local" // OpenClaw uses its own config structure
) {
    if (!jsonOutput) {
        console.log();
    ...
    const selectedSkills = setupMode === "mcp-only"
        ? []
 ...
 * Claude Code wizard
 */
export async function claudeCodeWizard(platform, skipPrompts = false, defaultNetwork = "sepolia", jsonOutput = false, customSkills, defaultConfigScope = "local") {
    if (!jsonOutput) {
        console.log();
        console.log(pc.cyan(`Setting up Starknet for ${platform.name}...`));
        console.log();
 ...
 * Cursor wizard
 */
export async function cursorWizard(platform, skipPrompts = false, defaultNetwork = "sepolia", jsonOutput = false, customSkills, defaultConfigScope = "local") {
    if (!jsonOutput) {
        console.log();
        console.log(pc.cyan(`Setting up Starknet for ${platform.name}...`));
        console.log();
 ...
 * Daydreams wizard
 */
export async function daydreamsWizard(platform, skipPrompts = false, defaultNetwork = "sepolia", jsonOutput = false, customSkills, _configScope = "local" // Daydreams uses its own config structure
) {
    if (!jsonOutput) {
        console.log();
        console.log(pc.cyan(`Setting up Starknet for ${platform.name}...`));
 ...
 * Generic MCP wizard
 */
export async function genericMcpWizard(platform, skipPrompts = false, defaultNetwork = "sepolia", jsonOutput = false, customSkills, _configScope = "local" // Generic MCP uses project-local mcp.json
) {
    if (!jsonOutput) {
        console.log();
 ...
 */
export async function runWizard(platform, skipPrompts = false, defaultNetwork = "sepolia", jsonOutput = false, customSkills, configScope = "local") {
