 ...
 */
export function parseCredentialsArgs(args) {
    const result = {
        platform: undefined,
        fromEnv: false,
        fromReady: false,
 ...
 */
export function printCredentialsHelp() {
    console.log(`
${pc.bold("Usage:")}
 ...
 */
function printWalletExportGuide(wallet) {
    console.log();
 ...
 */
export function validateCredentials(address, privateKey, rpcUrl) {
    const errors = [];
    const warnings = [];
 ...
 */
function getCredentialStoragePath(platform, address) {
 ...
 */
function getStorageType(platform) {
 ...
 */
function saveCredentialsJson(filePath, address, privateKey, rpcUrl, network) {
 ...
 */
function saveCredentialsEnv(filePath, address, privateKey, rpcUrl) {
    const envContent = [];
...
function ensureGitignore(dir) {
    ...
    const hasEnv = lines.some((line) => line.trim() === ".env" || line.trim() === ".env*" || line.trim() === "*.env");
    if (!hasEnv) {
 ...
 */
function outputJsonResult(result) {
 ...
 */
export async function runCredentialsSetup(args) {
    ...
    if (args.fromEnv) {
        ...
        if (!args.jsonOutput) {
            if (envCreds.address || envCreds.privateKey || envCreds.rpcUrl) {
                ...
                if (envCreds.address)
                    console.log(pc.dim(`  Address: ${envCreds.address.slice(0, 10)}...`));
                if (envCreds.privateKey)
                    console.log(pc.dim("  Private key: ****"));
                if (envCreds.rpcUrl)
                    console.log(pc.dim(`  RPC URL: ${envCreds.rpcUrl}`));
                console.log();
            }
            else {
    ...
    const rpcResponse = await prompts({
        ...
        validate: (value) => isValidRpcUrl(value) || "Invalid URL format (must be http:// or https://)",
    }, { onCancel });
    ...
    const addressResponse = await prompts({
        ...
        validate: (value) => isValidAddress(value) || "Invalid address format (expected 0x + 1-64 hex characters)",
    }, { onCancel });
    ...
    const keyResponse = await prompts({
        ...
        name: "privateKey",
        message: "Private key:",
        validate: (value) => isValidPrivateKey(value) || "Invalid key format (expected 0x + 1-64 hex characters)",
    }, { onCancel });
    ...
    let detectedNetwork = args.network;
