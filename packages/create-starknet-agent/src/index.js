...
import { detectPlatforms, formatDetectedPlatforms, getPlatformByType, isValidPlatformType, } from "./platform.js";
 ...
 */
function parseSkills(skillsArg) {
 ...
 */
async function selectPlatform(detectedPlatforms, platformOverride, skipPrompts = false) {
...
// Interactive prompts
async function getProjectConfig(initialName, initialTemplate, initialNetwork, initialPlatform, skipPrompts = false, initialSkills, initialConfigScope = "local") {
    // Detect platforms
    ...
    if (!initialName && selectedPlatform.type === "standalone") {
        questions.push({
            ...
            validate: (value) => isValidProjectName(value) || "Invalid name. Use letters, numbers, - or _",
        });
    }
    ...
    const network = initialNetwork || responses.network || "sepolia";
    ...
    if (selectedPlatform.type === "standalone" && network === "custom") {
        ...
        customRpcUrl = customResponse.customRpcUrl;
    }
    ...
    if (selectedPlatform.type === "standalone" && (template === "defi" || template === "full")) {
        ...
        defiProtocols = protocolResponse.protocols;
    }
    ...
    if (selectedPlatform.type === "standalone") {
        ...
        installDeps = installResponse.installDeps;
    }
    return {
        projectName,
        network,
...
async function createProject(config, jsonOutput = false) {
...
async function main() {
    ...
    const config = await getProjectConfig(parsed.projectName, parsed.template, parsed.network, parsed.platform, parsed.skipPrompts, parsed.skills, parsed.configScope);
    if (!config) {
        if (parsed.jsonOutput) {
            outputJson({
                success: false,
    ...
    if (config.platform?.type === "standalone") {
        ...
        const result = await runWizard(config.platform, parsed.skipPrompts, config.network, parsed.jsonOutput, config.skills, config.configScope || "local");
