...
describe("controller-cli skill acceptance", () => {
    ...
    const wrapper = path.join(repoRoot, "skills", "controller-cli", "scripts", "controller_safe.py");
    const validator = path.join(repoRoot, "skills", "controller-cli", "scripts", "validate_hex_address.py");
    it("appends --json and returns parsed JSON output", () => {
        ...
        fs.writeFileSync(stubController, `#!/usr/bin/env python3
import json, os, sys
...
`, { encoding: "utf-8" });
        fs.chmodSync(stubController, 0o755);
        const res = spawnSync("python3", [wrapper, "execute", "0xabc", "transfer", "0x1"], {
            env: {
                ...process.env,
                PATH: `${tmp}:${process.env.PATH ?? ""}`,
                CALLED_FILE: calledFile,
...
`, { encoding: "utf-8" });
        fs.chmodSync(stubController, 0o755);
        const res = spawnSync("python3", [wrapper, "call", "0x1", "balance_of", "0x2", "--chain-id", "SN_SEPOLIA"], {
            env: {
                ...process.env,
                PATH: `${tmp}:${process.env.PATH ?? ""}`,
                CALLED_FILE: calledFile,
