...
import { isValidAddress, isValidPrivateKey, isValidRpcUrl, validateCredentials, parseCredentialsArgs, } from "../credentials.js";
describe("credentials", () => {
    describe("isValidAddress", () => {
        it("accepts valid Starknet addresses", () => {
            // Full length address
            expect(isValidAddress("0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7")).toBe(true);
    ...
    describe("isValidPrivateKey", () => {
        it("accepts valid private keys", () => {
            // Full length key
            expect(isValidPrivateKey("0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7")).toBe(true);
            // Shorter key
    ...
    describe("validateCredentials", () => {
        it("validates correct credentials", () => {
            const result = validateCredentials("0x1234567890abcdef", "0xabcdef1234567890", "https://starknet-sepolia.g.alchemy.com/starknet/version/rpc/v0_7/test");
            expect(result.valid).toBe(true);
            expect(result.errors).toHaveLength(0);
        });
        it("reports invalid address", () => {
            const result = validateCredentials("invalid-address", "0xabcdef1234567890", "https://starknet-sepolia.g.alchemy.com/starknet/version/rpc/v0_7/test");
            expect(result.valid).toBe(false);
            expect(result.errors).toContain("Invalid address format (expected 0x + 1-64 hex characters)");
        });
        it("reports invalid private key", () => {
            const result = validateCredentials("0x1234567890abcdef", "invalid-key", "https://starknet-sepolia.g.alchemy.com/starknet/version/rpc/v0_7/test");
            expect(result.valid).toBe(false);
            expect(result.errors).toContain("Invalid private key format (expected 0x + 1-64 hex characters)");
        });
        ...
        it("warns about missing RPC URL", () => {
            const result = validateCredentials("0x1234567890abcdef", "0xabcdef1234567890", "");
            expect(result.valid).toBe(true);
            expect(result.warnings).toContain("No RPC URL provided, will use default public RPC");
        });
        it("reports invalid RPC URL", () => {
            const result = validateCredentials("0x1234567890abcdef", "0xabcdef1234567890", "not-a-url");
            expect(result.valid).toBe(false);
