 ...
 */
function sanitizeVerificationResultForOutput(result) {
    return {
        ...result,
        credentials: {
            ...result.credentials,
 ...
 */
async function checkMcpServer(platform, verbose) {
    const result = {
        configExists: false,
    ...
    if (fs.existsSync(configPath)) {
        ...
        try {
            ...
            if (config.mcpServers?.starknet) {
                ...
                const versionArg = args.find((a) => a.includes("@starknet-agentic/mcp-server"));
                if (versionArg) {
 ...
 */
async function pingMcpServer(verbose) {
    ...
    if (!result.privateKeyPresent || !result.accountAddressPresent) {
        ...
        if (fs.existsSync(expandedEnvPath)) {
            try {
                ...
                if (!result.accountAddressPresent &&
 ...
 */
function detectNetworkFromRpcUrl(url) {
 ...
 */
async function checkE2E(credentials, verbose) {
    const result = {
        attempted: false,
        success: false,
    };
 ...
 */
async function fetchBalance(accountAddress, rpcUrl, verbose) {
 ...
 */
async function callBalanceOf(rpcUrl, tokenAddress, accountAddress, verbose) {
    const payload = {
        jsonrpc: "2.0",
        id: 1,
...
export async function runVerification(args) {
    ...
    // End-to-end test
    let e2eResult = { attempted: false, success: false };
    if (!args.skipE2E) {
        if (!args.jsonOutput) {
