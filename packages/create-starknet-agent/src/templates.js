...
function generateEnvExample(config) {
    const rpcUrl = config.network === "custom"
...
`;
    if (config.template !== "minimal") {
        const avnuUrls = config.network === "sepolia" ? AVNU_URLS.sepolia : AVNU_URLS.mainnet;
        env += `
# AVNU DEX Aggregator
...
function generateMinimalAgent(config) {
...
function generateDeFiConfig(config) {
    const tokens = config.network === "sepolia"
        ? TOKEN_ADDRESSES.sepolia
        : TOKEN_ADDRESSES.mainnet;
...
function generateDeFiAgent(config) {
    return `/**
...
function generateIdentityModule() {
    return `/**
...
function generateFullAgent(config) {
    return `/**
