...
function extractTraceId(requestInit) {
...
function buildAllowResponse(traceId) {
    return {
...
function buildFetchJsonResponse(payload, status = 200) {
    return {
...
function buildProxySuccessResponse(requestInit, overrides = {}) {
...
function mockProxySuccessFetch(overrides = {}) {
    return vi.fn().mockImplementation(async (_url, requestInit) => {
...
describe("KeyringProxySigner", () => {
    ...
    it("matches canonical session-signature-v2 vector domain hash and runtime message hash", () => {
        const vectors = JSON.parse(fs.readFileSync(new URL("../../../../spec/session-signature-v2.json", import.meta.url), "utf8"));
        const vector = vectors.vectors.find((item) => item.id === "outside_execution_single_call_sepolia_v2");
        expect(vector).toBeDefined();
        const typed = outsideExecution.getTypedData(vector.domain.chainId, {
        ...
        }, vector.message.nonce, vector.message.calls.map((call) => ({
            contractAddress: call.to,
            entrypoint: call.selector,
            calldata: call.calldata,
        })), "2");
        const domainType = typed.types.StarknetDomain
            ? "StarknetDomain"
            : "StarkNetDomain";
        const computedDomainHash = typedData.getStructHash(typed.types, domainType, typed.domain, typed.domain?.revision);
        const computedMessageHash = typedData.getMessageHash(typed, num.toHex(BigInt(vector.accountAddress)));
        const expectedRuntimeMessageHash = "0x31a7322b5e322da06a35b192db191a1c218b6924a68e257bf15f92264ba8f09";
    ...
    it("signs transactions through keyring proxy with HMAC headers", async () => {
        ...
        const expectedPayload = `${headers["x-keyring-timestamp"]}.${headers["x-keyring-nonce"]}.POST.` +
    ...
    it("surfaces keyring proxy policy errors", async () => {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "set_agent_id", calldata: [] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow("selector denied");
    ...
    it("returns timeout error when proxy request aborts", async () => {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow("Keyring proxy request timed out");
    });
    it("rejects proxy signatures that are not 4-felt session signatures", async () => {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow("expected [pubkey, r, s, valid_until]");
    });
    it("rejects proxy responses without signerProvider", async () => {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow("signerProvider must be local or dfns");
    });
    it("rejects proxy responses without sessionPublicKey", async () => {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow("sessionPublicKey is required");
    });
    it("rejects proxy signatures when signatureMode is not v2_snip12", async () => {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow("signatureMode must be v2_snip12");
    });
    it.each([
        {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow(expectedError);
    });
    it("rejects proxy signatures when audit.keyId does not match requested keyId", async () => {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow("audit.keyId does not match requested keyId");
    });
    it("rejects proxy signatures when sessionPublicKey mismatches signature pubkey", async () => {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow("sessionPublicKey does not match signature pubkey");
    });
    it("rejects proxy signatures when valid_until does not match requested window", async () => {
        ...
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" })).rejects.toThrow("signature valid_until does not match requested window");
    });
    it("rejects unexpected session pubkey changes across requests", async () => {
        ...
        await signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" });
        await expect(signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x2"] }], { chainId: "0x1", nonce: "0x2" })).rejects.toThrow("session public key changed unexpectedly");
    });
    it("rejects incomplete mTLS client configuration", async () => {
        expect(() => new KeyringProxySigner({
    ...
    it("uses mTLS transport when TLS client material is configured", async () => {
        ...
        const signature = await signer.signTransaction([{ contractAddress: "0x111", entrypoint: "transfer", calldata: ["0x1"] }], { chainId: "0x1", nonce: "0x1" });
        expect(signature).toEqual(["0x123", "0xaaa", "0xbbb", "0x698f136c"]);
