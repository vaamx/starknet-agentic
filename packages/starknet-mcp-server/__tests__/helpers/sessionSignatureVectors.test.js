...
const STARKNET_DOMAIN_TYPE_HASH_REV1 = "0x1ff2f602e42168014d405a94f75e8a93d640751d71d16311266e140d8b0a210";
...
function computeV2Hashes(payload) {
    ...
    const messageHash = normalizeFelt(hash.computePoseidonHashOnElements([
    ...
    for (const vector of vectors.sessionVectors) {
        it(`matches conformance vector ${vector.id}`, () => {
            if (vector.mode === "v1_legacy") {
                ...
                expect(signingHash, `${vector.id}: signing hash mismatch`).toBe(normalizeFelt(vector.expected.signingMessageHash));
                expect(verificationHash, `${vector.id}: verification hash mismatch`).toBe(normalizeFelt(vector.expected.verificationMessageHash));
                expect(shouldVerify, `${vector.id}: verification expectation mismatch`).toBe(vector.expected.shouldVerify);
                return;
            }
            ...
            expect(expectedSigningDomainHash, `${vector.id}: missing expected signingDomainHash for v2 vector`).toBeDefined();
            expect(expectedVerificationDomainHash, `${vector.id}: missing expected verificationDomainHash for v2 vector`).toBeDefined();
            if (!expectedSigningDomainHash || !expectedVerificationDomainHash) {
                return;
            }
            expect(signing.domainHash, `${vector.id}: signing domain hash mismatch`).toBe(normalizeFelt(expectedSigningDomainHash));
            expect(verification.domainHash, `${vector.id}: verification domain hash mismatch`).toBe(normalizeFelt(expectedVerificationDomainHash));
            expect(signing.messageHash, `${vector.id}: signing hash mismatch`).toBe(normalizeFelt(vector.expected.signingMessageHash));
            expect(verification.messageHash, `${vector.id}: verification hash mismatch`).toBe(normalizeFelt(vector.expected.verificationMessageHash));
            expect(shouldVerify, `${vector.id}: verification expectation mismatch`).toBe(vector.expected.shouldVerify);
        });
    }
});
