...
import { getVTokenAddress, buildDepositCalls, buildWithdrawCalls, VESU_POOL_FACTORY, VESU_PRIME_POOL, } from "../../src/helpers/vesu.js";
const TOKENS = {
...
const MOCK_VTOKEN_STRK = "0x01a1b2c3d4e5f60708192a3b4c5d6e7f8090a1b2c3d4e5f60708192a3b4c5d6e";
...
describe("vesu helper", () => {
    ...
    describe("getVTokenAddress", () => {
        ...
        it("calls PoolFactory.v_token_for_asset with correct args", async () => {
            ...
            const provider = { callContract: mockCallContract };
            const result = await getVTokenAddress(provider, VESU_PRIME_POOL, TOKENS.STRK);
        ...
        it("returns vToken for USDC", async () => {
            const usdcVToken = "0x02b2c3d4e5f60708192a3b4c5d6e7f8090a1b2c3d4e5f60708192a3b4c5d6e";
            mockCallContract.mockResolvedValue([usdcVToken]);
            const provider = { callContract: mockCallContract };
            const result = await getVTokenAddress(provider, VESU_PRIME_POOL, TOKENS.USDC);
        ...
        it("throws when PoolFactory returns empty", async () => {
            mockCallContract.mockResolvedValue([]);
            const provider = { callContract: mockCallContract };
            await expect(getVTokenAddress(provider, VESU_PRIME_POOL, TOKENS.STRK)).rejects.toThrow("vToken not found");
        });
        it("propagates RPC errors", async () => {
            mockCallContract.mockRejectedValue(new Error("RPC failed"));
            const provider = { callContract: mockCallContract };
            await expect(getVTokenAddress(provider, VESU_PRIME_POOL, TOKENS.STRK)).rejects.toThrow("RPC failed");
        });
    });
    describe("buildDepositCalls", () => {
        it("returns approve + deposit in correct order", () => {
            const amountWei = 1000000000000000000n; // 1 STRK
            const calls = buildDepositCalls(TOKENS.STRK, MOCK_VTOKEN_STRK, amountWei, RECEIVER);
        ...
        it("approve calldata has spender and amount (u256)", () => {
            const calls = buildDepositCalls(TOKENS.STRK, MOCK_VTOKEN_STRK, 1000n, RECEIVER);
            const approveCalldata = calls[0].calldata;
        ...
        it("rejects zero amount", () => {
            expect(() => buildDepositCalls(TOKENS.STRK, MOCK_VTOKEN_STRK, 0n, RECEIVER)).toThrow("amount must be positive");
        });
        it("rejects negative amount", () => {
            expect(() => buildDepositCalls(TOKENS.STRK, MOCK_VTOKEN_STRK, -1n, RECEIVER)).toThrow("amount must be positive");
        });
        it("handles large amounts (u256)", () => {
            const large = (1n << 128n) - 1n;
            const calls = buildDepositCalls(TOKENS.STRK, MOCK_VTOKEN_STRK, large, RECEIVER);
            expect(calls).toHaveLength(2);
            expect(calls[0].calldata).toBeDefined();
            expect(calls[1].calldata).toBeDefined();
        });
    ...
    describe("buildWithdrawCalls", () => {
        it("returns single withdraw call", () => {
            const amountWei = 1000000n; // 1 USDC
            const calls = buildWithdrawCalls(MOCK_VTOKEN_STRK, amountWei, RECEIVER, OWNER);
        ...
        it("rejects zero amount", () => {
            expect(() => buildWithdrawCalls(MOCK_VTOKEN_STRK, 0n, RECEIVER, OWNER)).toThrow("amount must be positive");
        });
        it("rejects negative amount", () => {
            expect(() => buildWithdrawCalls(MOCK_VTOKEN_STRK, -1n, RECEIVER, OWNER)).toThrow("amount must be positive");
        });
        it("owner and receiver can be same address", () => {
            const calls = buildWithdrawCalls(MOCK_VTOKEN_STRK, 100n, RECEIVER, RECEIVER);
            expect(calls).toHaveLength(1);
            const calldata = calls[0].calldata;
    ...
    describe("edge cases and invalid inputs", () => {
        it("buildDepositCalls throws on invalid asset address", () => {
            expect(() => buildDepositCalls("0xinvalid", MOCK_VTOKEN_STRK, 100n, RECEIVER)).toThrow();
        });
        it("buildDepositCalls throws on invalid vToken address", () => {
            expect(() => buildDepositCalls(TOKENS.STRK, "not-an-address", 100n, RECEIVER)).toThrow();
        });
        it("buildDepositCalls throws on invalid receiver", () => {
            expect(() => buildDepositCalls(TOKENS.STRK, MOCK_VTOKEN_STRK, 100n, "")).toThrow();
        });
        it("buildWithdrawCalls throws on invalid vToken address", () => {
            expect(() => buildWithdrawCalls("0xinvalid", 100n, RECEIVER, OWNER)).toThrow();
        });
        it("getVTokenAddress rejects empty calldata response", async () => {
            const mockCall = vi.fn().mockResolvedValue([]);
            const provider = { callContract: mockCall };
            await expect(getVTokenAddress(provider, VESU_PRIME_POOL, TOKENS.STRK)).rejects.toThrow("vToken not found");
        });
        it("getVTokenAddress handles result wrapped in object", async () => {
            const mockCall = vi.fn().mockResolvedValue({ result: [MOCK_VTOKEN_STRK] });
            const prov = { callContract: mockCall };
    ...
    describe("fuzz / property-based", () => {
        it("buildDepositCalls amount equals parsed input for various values", () => {
            ...
            for (const parsed of amounts) {
                const calls = buildDepositCalls(TOKENS.STRK, MOCK_VTOKEN_STRK, parsed, RECEIVER);
                const depositCalldata = calls[1].calldata;
        ...
        it("buildWithdrawCalls preserves amount through u256 encoding", () => {
            const amounts = [1n, 1000n, 1000000n, 1000000000000000000n];
            for (const amt of amounts) {
                const calls = buildWithdrawCalls(MOCK_VTOKEN_STRK, amt, RECEIVER, OWNER);
                const calldata = calls[0].calldata;
