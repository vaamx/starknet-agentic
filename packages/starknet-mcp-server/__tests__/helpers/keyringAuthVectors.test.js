...
import { InMemoryNonceStore, buildKeyringSigningPayload, validateKeyringRequestAuth, } from "../../src/helpers/keyringAuthContract.js";
const vectors = JSON.parse(fs.readFileSync(new URL("../../../../spec/signer-auth-v1.json", import.meta.url), "utf8"));
    ...
    for (const vector of vectors.vectors) {
        it(vector.id, async () => {
            ...
            for (const step of vector.steps) {
                ...
                const signature = step.overrideSignature ??
                ...
                if (step.expect.ok) {
                    ...
                    if (!result.ok) {
                        expect(result.errorCode, `${vector.id} step ${stepIndex} errorCode`).toBe(step.expect.errorCode);
                    }
                }
                stepIndex += 1;
            }
