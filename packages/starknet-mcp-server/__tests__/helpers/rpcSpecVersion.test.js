...
describe("resolveRpcSpecVersion", () => {
    ...
    it("rejects malformed or unsupported versions", () => {
        expect(() => resolveRpcSpecVersion("0.10foo")).toThrow(/Unsupported STARKNET_RPC_SPEC_VERSION/);
        expect(() => resolveRpcSpecVersion("0.9beta")).toThrow(/Unsupported STARKNET_RPC_SPEC_VERSION/);
        expect(() => resolveRpcSpecVersion("1.0.0")).toThrow(/Unsupported STARKNET_RPC_SPEC_VERSION/);
    });
});
