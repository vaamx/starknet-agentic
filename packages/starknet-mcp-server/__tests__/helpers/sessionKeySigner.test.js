...
describe("SessionKeySigner", () => {
    ...
    describe("signTransaction", () => {
        ...
        it("returns a 4-element signature array", async () => {
            ...
            const sig = await signer.signTransaction(calls, makeTransactionDetail());
        ...
        it("signature matches independent hash computation", async () => {
            ...
            const sig = await signer.signTransaction(calls, detail);
        ...
        it("different calls produce different signatures", async () => {
            const detail = makeTransactionDetail();
            const sig1 = await signer.signTransaction([makeCall("0xAAA", "transfer", ["0x1"])], detail);
            const sig2 = await signer.signTransaction([makeCall("0xAAA", "transfer", ["0x2"])], detail);
        ...
        it("different nonces produce different signatures", async () => {
            const calls = [makeCall("0xAAA", "transfer")];
            const sig1 = await signer.signTransaction(calls, makeTransactionDetail({ nonce: "0x1" }));
            const calls2 = [makeCall("0xAAA", "transfer")];
            const sig2 = await signer.signTransaction(calls2, makeTransactionDetail({ nonce: "0x2" }));
            expect(sig1[1] !== sig2[1] || sig1[2] !== sig2[2]).toBe(true);
        });
        ...
        it("hex entrypoint and name entrypoint produce same signature", async () => {
            const selectorHex = hash.getSelectorFromName("transfer");
            const detail = makeTransactionDetail();
            const sigName = await signer.signTransaction([makeCall("0xAAA", "transfer")], detail);
            const sigHex = await signer.signTransaction([makeCall("0xAAA", selectorHex)], detail);
        ...
        it("throws when accountAddress is missing", async () => {
            ...
            await expect(signer.signTransaction(calls, badDetail)).rejects.toThrow("SessionKeySigner: cannot determine account address");
        });
        ...
        it("different chain IDs produce different signatures", async () => {
            const calls = [makeCall("0xAAA", "transfer")];
            const sig1 = await signer.signTransaction(calls, makeTransactionDetail({ chainId: "0x534e5f5345504f4c4941" }));
            const calls2 = [makeCall("0xAAA", "transfer")];
            const sig2 = await signer.signTransaction(calls2, makeTransactionDetail({ chainId: "0x534e5f4d41494e" }));
            expect(sig1[1] !== sig2[1] || sig1[2] !== sig2[2]).toBe(true);
        });
        it("different account addresses produce different signatures", async () => {
            const calls = [makeCall("0xAAA", "transfer")];
            const sig1 = await signer.signTransaction(calls, makeTransactionDetail({ accountAddress: "0x111" }));
            const calls2 = [makeCall("0xAAA", "transfer")];
            const sig2 = await signer.signTransaction(calls2, makeTransactionDetail({ accountAddress: "0x222" }));
            expect(sig1[1] !== sig2[1] || sig1[2] !== sig2[2]).toBe(true);
        });
    });
    ...
    describe("signDeployAccountTransaction", () => {
        it("throws — session keys cannot deploy accounts", async () => {
            await expect(signer.signDeployAccountTransaction({})).rejects.toThrow("Session key signer cannot sign deploy account transactions");
        });
    });
    // ── signDeclareTransaction ───────────────────────────────────────────
    describe("signDeclareTransaction", () => {
        it("throws — session keys cannot declare classes", async () => {
            await expect(signer.signDeclareTransaction({})).rejects.toThrow("Session key signer cannot sign declare transactions");
        });
    });
    // ── signMessage ──────────────────────────────────────────────────────
    ...
    describe("cross-key confusion resistance", () => {
        it("signature from key A does not verify with key B", async () => {
            ...
            const sig = await signerA.signTransaction(calls, detail);
            // Recompute hash
            const hashData = [];
            ...
            const sigObj = new ec.starkCurve.Signature(BigInt(sig[1]), BigInt(sig[2]));
    ...
    describe("edge cases", () => {
        ...
        it("handles numeric calldata (non-string)", async () => {
            ...
            const calls = [{ contractAddress: "0xAAA", entrypoint: "transfer", calldata: [1, 2, 3] }];
            const detail = {
                accountAddress: "0x123",
                chainId: "0x534e5f5345504f4c4941",
                nonce: "0x1",
        ...
        it("produces deterministic signatures for identical inputs", async () => {
            ...
            const sig1 = await signer.signTransaction(calls1, detail);
            const sig2 = await signer.signTransaction(calls2, detail);
        ...
        it("validUntil = 0 produces valid signature format", async () => {
            ...
            const sig = await zeroSigner.signTransaction(calls, detail);
            expect(sig).toHaveLength(4);
