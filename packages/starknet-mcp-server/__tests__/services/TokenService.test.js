...
describe("TokenService", () => {
    ...
    describe("on-chain fallback", () => {
        it("should return cached decimals without avnu or on-chain call", async () => {
            ...
            const decimals = await service.getDecimalsAsync("0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7");
            expect(decimals).toBe(18);
        ...
        it("should throw if avnu fails and no provider configured", async () => {
            ...
            await expect(service.getDecimalsAsync(unknownAddr)).rejects.toThrow("no RPC provider configured");
        });
        it("should fetch from contract when avnu fails and provider is configured", async () => {
            ...
            vi.mocked(Contract).mockImplementation(function MockContract() { return mockContract; });
            // Configure provider
            const mockProvider = {};
        ...
        it("should throw when all contract calls fail (invalid ERC20)", async () => {
            ...
            await expect(service.getTokenByAddress(unknownAddr)).rejects.toThrow("does not appear to be a valid ERC20 token");
        });
