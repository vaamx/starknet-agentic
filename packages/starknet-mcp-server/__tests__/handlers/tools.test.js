...
const mockEnv = {
    ...
    ERC8004_IDENTITY_REGISTRY_ADDRESS: "0x1111111111111111111111111111111111111111111111111111111111111111",
};
...
const mockValidateAndParseAddress = vi.fn((addr) => addr.toLowerCase().padStart(66, "0x".padEnd(66, "0")));
vi.mock("starknet", () => ({
    ...
    hash: {
        getSelectorFromName: vi.fn((name) => `selector:${name}`),
    },
}));
// Mock avnu-sdk
...
let capturedToolHandler = null;
let capturedListHandler = null;
...
describe("MCP Tool Handlers", () => {
    ...
    describe("starknet_register_agent", () => {
        it("is listed when ERC8004_IDENTITY_REGISTRY_ADDRESS is configured", async () => {
            ...
            const toolNames = resp.tools.map((t) => t.name);
    ...
    describe("starknet_transfer", () => {
        ...
        it("executes transfer with gasfree mode (no API key)", async () => {
            ...
            expect(mockExecutePaymasterTransaction).toHaveBeenCalledWith(expect.any(Array), expect.objectContaining({
    ...
    describe("starknet_vesu_deposit", () => {
        const VTOKEN_STRK = "0x01a1b2c3d4e5f60708192a3b4c5d6e7f8090a1b2c3d4e5f60708192a3b4c5d6e";
        ...
        it("uses custom pool when provided", async () => {
            ...
            const result = parseResponse(response);
            expect(result.success).toBe(true);
            expect(mockCallContract).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("starknet_swap", () => {
        ...
        it("respects slippage parameter", async () => {
            ...
            expect(mockQuoteToCalls).toHaveBeenCalledWith(expect.objectContaining({ slippage: 0.02 }), expect.any(Object));
        });
    });
    ...
    describe("starknet_estimate_fee", () => {
        ...
        it("passes calldata correctly", async () => {
            ...
            expect(mockEstimateInvokeFee).toHaveBeenCalledWith(expect.objectContaining({
                entrypoint: "call_with_args",
                calldata: ["0x1", "0x2"],
            }));
        });
...
describe("MCP Startup Guardrails", () => {
    ...
    it("fails startup in production when signer mode is direct", async () => {
        ...
        vi.resetModules();
        await expect(import("../../src/index.js")).rejects.toThrow("Production mode requires STARKNET_SIGNER_MODE=proxy");
    });
    ...
    it("fails startup in proxy mode when keyring auth is missing", async () => {
        ...
        vi.resetModules();
        await expect(import("../../src/index.js")).rejects.toThrow("Missing keyring proxy configuration for STARKNET_SIGNER_MODE=proxy");
    });
    it("fails startup in production proxy mode if direct private key is still set", async () => {
        ...
        vi.resetModules();
        await expect(import("../../src/index.js")).rejects.toThrow("STARKNET_PRIVATE_KEY must not be set in production when STARKNET_SIGNER_MODE=proxy");
    });
    it("fails startup in production proxy mode when proxy URL is non-https and non-loopback", async () => {
        ...
        delete process.env.STARKNET_PRIVATE_KEY;
        vi.resetModules();
        await expect(import("../../src/index.js")).rejects.toThrow("Production proxy mode requires KEYRING_PROXY_URL to use https unless loopback is used");
    });
    it("fails startup in production proxy mode when mTLS client cert config is missing", async () => {
        ...
        vi.resetModules();
        await expect(import("../../src/index.js")).rejects.toThrow("Production proxy mode requires KEYRING_TLS_CLIENT_CERT_PATH, KEYRING_TLS_CLIENT_KEY_PATH, and KEYRING_TLS_CA_PATH for mTLS");
    });
});
describe("Tool list", () => {
    beforeEach(async () => {
