...
vi.mock("starknet", async (importOriginal) => {
    ...
    return {
        ...actual,
        Contract: vi.fn().mockImplementation(function MockContract({ address }) {
...
describe("fetchTokenBalances", () => {
    ...
    it("uses BalanceChecker when available and returns balance_checker method", async () => {
        ...
        const result = await fetchTokenBalances("0x123", ["ETH"], [TOKENS.ETH], mockProvider);
    ...
    it("propagates error when both methods fail", async () => {
        ...
        await expect(fetchTokenBalances("0x123", ["ETH"], [TOKENS.ETH], mockProvider)).rejects.toThrow("RPC failed");
    });
    ...
    it("returns zero balance for tokens not in BalanceChecker response", async () => {
        ...
        const result = await fetchTokenBalances("0x123", ["ETH", "STRK"], [TOKENS.ETH, TOKENS.STRK], mockProvider);
