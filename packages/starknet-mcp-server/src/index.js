...
import { Account, RpcProvider, PaymasterRpc, CallData, cairo, byteArray, ETransactionVersion, hash, validateAndParseAddress, } from "starknet";
import { randomBytes } from "node:crypto";
import { resolveTokenAddressAsync, validateTokensInputAsync, } from "./utils.js";
import { getTokenService, configureTokenServiceProvider, TOKENS, STATIC_TOKENS } from "./services/index.js";
import { fetchTokenBalance, fetchTokenBalances, } from "./helpers/balance.js";
import { getVTokenAddress, buildDepositCalls, buildWithdrawCalls, VESU_PRIME_POOL, } from "./helpers/vesu.js";
...
const env = envSchema.parse({
    ...
    AVNU_PAYMASTER_FEE_MODE: process.env.AVNU_PAYMASTER_FEE_MODE,
...
if (isProductionRuntime && signerMode !== "proxy") {
    throw new Error("Production mode requires STARKNET_SIGNER_MODE=proxy to prevent in-process private key signing");
}
...
if (signerMode === "proxy") {
    if (!env.KEYRING_PROXY_URL || !env.KEYRING_HMAC_SECRET) {
        throw new Error("Missing keyring proxy configuration for STARKNET_SIGNER_MODE=proxy (KEYRING_PROXY_URL, KEYRING_HMAC_SECRET)");
    }
    if (isProductionRuntime) {
        const proxyUrl = new URL(env.KEYRING_PROXY_URL);
        const isLoopback = proxyUrl.hostname === "127.0.0.1" ||
            proxyUrl.hostname === "localhost" ||
            proxyUrl.hostname === "::1" ||
            proxyUrl.hostname === "[::1]";
        if (proxyUrl.protocol !== "https:" && !isLoopback) {
            throw new Error("Production proxy mode requires KEYRING_PROXY_URL to use https unless loopback is used");
        }
        if (!isLoopback) {
            if (!env.KEYRING_TLS_CLIENT_CERT_PATH ||
                !env.KEYRING_TLS_CLIENT_KEY_PATH ||
                !env.KEYRING_TLS_CA_PATH) {
                throw new Error("Production proxy mode requires KEYRING_TLS_CLIENT_CERT_PATH, KEYRING_TLS_CLIENT_KEY_PATH, and KEYRING_TLS_CA_PATH for mTLS");
            }
        }
    }
    if (isProductionRuntime && env.STARKNET_PRIVATE_KEY) {
        throw new Error("STARKNET_PRIVATE_KEY must not be set in production when STARKNET_SIGNER_MODE=proxy");
    }
}
...
if (isProductionRuntime) {
    ...
    const isLoopback = rpcUrl.hostname === "127.0.0.1" ||
        rpcUrl.hostname === "localhost" ||
        rpcUrl.hostname === "::1" ||
        rpcUrl.hostname === "[::1]";
    if (rpcUrl.protocol !== "https:" && !isLoopback) {
        throw new Error("Production mode requires STARKNET_RPC_URL to use HTTPS to protect transaction data in transit.");
    }
}
...
const paymasterFeeMode = env.AVNU_PAYMASTER_FEE_MODE ?? (env.AVNU_PAYMASTER_API_KEY ? "sponsored" : "default");
...
const accountSigner = signerMode === "proxy"
...
function parseAddress(name, value) {
    try {
        ...
        throw new Error(`${name} is not a valid Starknet address: "${value}". ` +
            "Expected a hex string starting with 0x.");
    }
}
const MAX_CALLDATA_LEN = 256;
function parseCalldata(name, calldata) {
...
function validateEntrypoint(name, value) {
 ...
 */
function assertQuoteNotExpired(quote) {
    if (quote.expiry != null && quote.expiry > 0) {
        const nowSec = Math.floor(Date.now() / 1000);
        if (nowSec >= quote.expiry) {
            throw new Error(`Swap quote has expired (expiry=${quote.expiry}, now=${nowSec}). ` +
...
function priceImpactWarning(priceImpact) {
    if (priceImpact != null && Math.abs(priceImpact) >= HIGH_PRICE_IMPACT_THRESHOLD) {
        return (`WARNING: Price impact is ${priceImpact.toFixed(2)}% which exceeds the ${HIGH_PRICE_IMPACT_THRESHOLD}% threshold. ` +
...
function parseDeployResultFromReceipt(receipt, factoryAddress) {
 ...
 */
async function executeTransaction(calls, gasfree, gasToken = TOKENS.STRK) {
    if (!gasfree) {
    ...
    const result = await account.executePaymasterTransaction(callsArray, paymasterDetails, estimation.suggested_max_fee_in_gas_token);
    return result.transaction_hash;
}
// MCP Server setup
...
const tools = [
    {
        ...
        description: "Get token balance for an address on Starknet. Supports ETH, STRK, USDC, USDT, or any token address. For multiple tokens, use starknet_get_balances instead.",
        inputSchema: {
            type: "object",
            properties: {
                address: {
    ...
    },
    {
        ...
        description: "Get multiple token balances for an address in a single RPC call. More efficient than calling starknet_get_balance multiple times. Supports ETH, STRK, USDC, USDT, or any token addresses.",
        inputSchema: {
            type: "object",
            properties: {
                address: {
    ...
    {
        name: "starknet_swap",
        description: "Execute a token swap on Starknet using avnu aggregator for best prices. Supports gasfree mode where gas is paid via paymaster.",
        inputSchema: {
            type: "object",
            properties: {
                sellToken: {
    ...
    },
    {
        ...
        description: "Supply assets to Vesu V2 lending pool (ERC-4626). Uses Prime pool by default. Requires approve + deposit. Supports gasfree mode.",
        inputSchema: {
            type: "object",
            properties: {
                token: {
    ...
    },
    {
        ...
        description: "Withdraw assets from Vesu V2 lending pool. Withdraws underlying assets (not shares). Supports gasfree mode.",
        inputSchema: {
            type: "object",
            properties: {
                token: {
    ...
    },
    {
        ...
        description: "Get lending positions (vToken balances and converted assets) for the agent's address in Vesu V2 pools.",
        inputSchema: {
            type: "object",
            properties: {
                tokens: {
    ...
    },
    {
        ...
        description: "Build unsigned Starknet calls without executing. Returns a JSON array of Call objects compatible with starknet.js account.execute() and Cartridge Controller. Use this when you need to compose calls for external signing (e.g., session keys, hardware wallets, multisig).",
        inputSchema: {
            type: "object",
            properties: {
                calls: {
...
if (signerMode === "direct") {
    tools.push({
        ...
        description: "Sign a base64 PAYMENT-REQUIRED header containing Starknet typedData, return a base64 PAYMENT-SIGNATURE header value.",
        inputSchema: {
            type: "object",
            properties: {
...
// ── Session key management tools ────────────────────────────────────
{
    ...
    description: "Register a session key on a SessionAccount (chipi-pay fork). Owner-only operation. The session key gets time-limited, call-count-limited access with optional selector whitelist.",
    inputSchema: {
        type: "object",
        properties: {
            accountAddress: {
            ...
            allowedEntrypoints: {
                type: "array",
                items: { type: "string" },
                description: "Optional array of allowed function selectors (felt strings). Empty = any non-admin selector on external contracts.",
                default: [],
            },
            gasfree: {
                type: "boolean",
    ...
    description: "Revoke a session key from a SessionAccount. Owner-only. Zeroes the session data and clears allowed entrypoints.",
    inputSchema: {
        type: "object",
        properties: {
            accountAddress: {
    ...
    description: "Read session key data from a SessionAccount. Returns valid_until, max_calls, calls_used, and allowed_entrypoints_len.",
    inputSchema: {
        type: "object",
        properties: {
            accountAddress: {
...
// ── Domain-specific unsigned call builders ──────────────────────────
{
    ...
    description: "Build unsigned ERC-20 transfer calls. Returns Call[] for external signing (session key, owner, hardware wallet).",
    inputSchema: {
        type: "object",
        properties: {
            tokenAddress: {
    ...
    description: "Build unsigned swap calls via AVNU. Returns approval + swap Call[] for external signing.",
    inputSchema: {
        type: "object",
        properties: {
            sellTokenAddress: {
...
if (env.AGENT_ACCOUNT_FACTORY_ADDRESS) {
    tools.push({
        ...
        description: "Deploy a new agent account via AgentAccountFactory. Requires caller-supplied public_key (no server-side key generation).",
        inputSchema: {
            type: "object",
            properties: {
                public_key: {
...
if (env.ERC8004_IDENTITY_REGISTRY_ADDRESS) {
    tools.push({
        ...
        description: "Register a new ERC-8004 agent identity in IdentityRegistry. Optionally provide token_uri. Returns tx hash and parsed agent_id (best-effort).",
        inputSchema: {
            type: "object",
            properties: {
                token_uri: {
    ...
    tools.push({
        ...
        description: "Set on-chain metadata for an ERC-8004 agent. Caller must be owner or approved for the agent_id. Standard keys: agentName, agentType, version, model, status, framework, capabilities, a2aEndpoint, moltbookId.",
        inputSchema: {
            type: "object",
            properties: {
                agent_id: {
                ...
                key: {
                    type: "string",
                    description: "Metadata key (e.g. 'agentName', 'capabilities'). 'agentWallet' is reserved and cannot be set here.",
                },
                value: {
                    type: "string",
    ...
    tools.push({
        ...
        description: "Read on-chain metadata for an ERC-8004 agent. Returns the value stored for the given key, or empty string if not set.",
        inputSchema: {
            type: "object",
            properties: {
                agent_id: {
...
function parseIdentityRegisteredFromReceipt(receipt, identityRegistryAddress) {
    const events = receipt
        ?.events;
    if (!events) {
        return { agentId: null };
...
async function parseAmount(amount, tokenAddress) {
    if (!/^\d+(\.\d+)?$/.test(amount)) {
        throw new Error(`Invalid amount "${amount}". Expected a non-negative decimal number (e.g. "1.5", "100").`);
    }
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const policyResult = policyGuard.evaluate(name, args);
    ...
    try {
        switch (name) {
            case "starknet_get_balance": {
                const { address = env.STARKNET_ACCOUNT_ADDRESS, token } = args;
            ...
            case "starknet_transfer": {
                const { recipient, token, amount, gasfree = false, gasToken } = args;
            ...
            case "starknet_call_contract": {
                const { contractAddress, entrypoint, calldata = [] } = args;
                    ...
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({
                                result: Array.isArray(result) ? result : result.result ?? result,
                                contractAddress,
                                entrypoint,
                            }, null, 2),
                        },
            ...
            case "starknet_invoke_contract": {
                const { contractAddress, entrypoint, calldata = [], gasfree = false, gasToken } = args;
                const validatedContractAddress = parseAddress("contractAddress", contractAddress);
                const validatedEntrypoint = validateEntrypoint("entrypoint", entrypoint);
                const validatedCalldata = parseCalldata("calldata", calldata);
                const gasTokenAddress = gasToken ? await resolveTokenAddressAsync(gasToken) : TOKENS.STRK;
            ...
            case "starknet_vesu_deposit": {
                const { token, amount, pool = VESU_PRIME_POOL, gasfree = false, gasToken } = args;
                ...
                const calls = buildDepositCalls(assetAddress, vTokenAddress, amountWei, account.address);
                const gasTokenAddress = gasToken ? await resolveTokenAddressAsync(gasToken) : TOKENS.STRK;
            ...
            case "starknet_vesu_withdraw": {
                ...
                const vTokenAddress = await getVTokenAddress(provider, poolAddress, assetAddress);
                const calls = buildWithdrawCalls(vTokenAddress, amountWei, account.address, account.address);
                const gasTokenAddress = gasToken ? await resolveTokenAddressAsync(gasToken) : TOKENS.STRK;
                const transactionHash = await executeTransaction(calls, gasfree, gasTokenAddress);
                await provider.waitForTransaction(transactionHash, { retries: TX_WAIT_RETRIES, retryInterval: TX_WAIT_INTERVAL_MS });
                return {
            ...
            case "starknet_vesu_positions": {
                const { tokens, address = env.STARKNET_ACCOUNT_ADDRESS, pool = VESU_PRIME_POOL } = args;
                ...
                for (let i = 0; i < tokens.length; i++) {
                    ...
                    const balanceArr = Array.isArray(balanceRaw) ? balanceRaw : balanceRaw.result ?? [];
                    const balanceVal = balanceArr.length >= 2
                        ? { low: BigInt(balanceArr[0]), high: BigInt(balanceArr[1] ?? 0) }
                        : balanceRaw?.balance ?? { low: 0n, high: 0n };
                    ...
                    if (shares > 0n) {
                        ...
                        const assetsArr = Array.isArray(assetsRaw) ? assetsRaw : assetsRaw.result ?? [];
            ...
            case "starknet_swap": {
                const { sellToken, buyToken, amount, slippage = 0.01, gasfree = false, gasToken } = args;
            ...
            case "starknet_build_calls": {
                ...
                const callKeys = validatedCalls.map((c) => `${c.contractAddress}:${c.entrypoint}:${c.calldata.join(",")}`);
                const seen = new Set();
                const duplicateIndices = [];
                for (let idx = 0; idx < callKeys.length; idx++) {
                    if (seen.has(callKeys[idx]))
                        duplicateIndices.push(idx);
                    seen.add(callKeys[idx]);
                }
                const warning = duplicateIndices.length > 0
            ...
            case "starknet_register_session_key": {
                const { accountAddress: rawAccountAddr, sessionPublicKey: rawSessionKey, validUntil, maxCalls, allowedEntrypoints: rawEntrypoints = [], gasfree = false, } = args;
                ...
                if (sessionAccountAddr.toLowerCase() !== configuredAccount.toLowerCase()) {
                    throw new Error(`accountAddress (${sessionAccountAddr}) does not match the configured MCP server account (${configuredAccount}). ` +
                ...
                if (validUntil > nowSec + MAX_SESSION_DURATION_SECS) {
                    throw new Error(`validUntil is too far in the future (max 30 days from now). ` +
                ...
                if (sessionKey === 0n) {
                    throw new Error("sessionPublicKey must be non-zero. A zero key creates an unusable session.");
                }
                ...
                if (sessionKey >= STARK_CURVE_ORDER) {
                    throw new Error("sessionPublicKey exceeds Stark curve order. This is not a valid public key.");
                }
            ...
            case "starknet_revoke_session_key": {
                const { accountAddress: rawAccountAddr, sessionPublicKey: rawSessionKey, gasfree = false, } = args;
                const sessionAccountAddr = parseAddress("accountAddress", rawAccountAddr);
                const sessionKey = parseFelt("sessionPublicKey", rawSessionKey);
                ...
                if (sessionAccountAddr.toLowerCase() !== configuredAccountRevoke.toLowerCase()) {
                    throw new Error(`accountAddress (${sessionAccountAddr}) does not match the configured MCP server account (${configuredAccountRevoke}). ` +
                        `Session key operations can only target the server's own account.`);
                }
                const revokeCall = {
                    contractAddress: sessionAccountAddr,
            ...
            case "starknet_get_session_data": {
                const { accountAddress: rawAccountAddr, sessionPublicKey: rawSessionKey, } = args;
                const sessionAccountAddr = parseAddress("accountAddress", rawAccountAddr);
                const sessionKey = parseFelt("sessionPublicKey", rawSessionKey);
            ...
            case "starknet_build_transfer_calls": {
                const { tokenAddress: rawToken, recipientAddress: rawRecipient, amount: rawAmount, } = args;
                ...
                const tokenService = getTokenService();
                const decimals = await tokenService.getDecimalsAsync(tokenAddress);
                const tokenSymbol = STATIC_TOKENS.find((t) => t.address.toLowerCase() === tokenAddress.toLowerCase())?.symbol ?? tokenAddress;
            ...
            case "starknet_build_swap_calls": {
                const { sellTokenAddress: rawSellToken, buyTokenAddress: rawBuyToken, sellAmount: rawSellAmount, signerAddress: rawSigner, slippageBps = 100, } = args;
                ...
                const sellSymbol = STATIC_TOKENS.find((t) => t.address.toLowerCase() === sellTokenAddress.toLowerCase())?.symbol ?? sellTokenAddress;
                const buySymbol = STATIC_TOKENS.find((t) => t.address.toLowerCase() === buyTokenAddress.toLowerCase())?.symbol ?? buyTokenAddress;
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "x402_starknet_sign_payment_required": {
                const { paymentRequiredHeader } = args;
                if (signerMode === "proxy") {
                    throw new Error("x402_starknet_sign_payment_required is disabled in STARKNET_SIGNER_MODE=proxy (requires direct private key signing)");
                }
                if (!env.STARKNET_RPC_URL || !env.STARKNET_ACCOUNT_ADDRESS || !env.STARKNET_PRIVATE_KEY) {
                    throw new Error("Missing required env vars for x402 signing (STARKNET_RPC_URL, STARKNET_ACCOUNT_ADDRESS, STARKNET_PRIVATE_KEY)");
                }
            ...
            case "starknet_deploy_agent_account": {
                ...
                const { public_key, token_uri, salt, gasfree = false } = args;
                ...
                const { accountAddress, agentId } = parseDeployResultFromReceipt(receipt, env.AGENT_ACCOUNT_FACTORY_ADDRESS);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "starknet_register_agent": {
                ...
                const { token_uri, gasfree = false } = args;
                const identity = parseAddress("ERC8004_IDENTITY_REGISTRY_ADDRESS", env.ERC8004_IDENTITY_REGISTRY_ADDRESS);
                const entrypoint = token_uri && token_uri.length > 0 ? "register_with_token_uri" : "register";
                const calldata = token_uri && token_uri.length > 0
            ...
            case "starknet_set_agent_metadata": {
                ...
                const { agent_id, key, value, gasfree = false } = args;
                if (!agent_id)
                    throw new Error("agent_id is required");
                if (!key || key.length === 0)
                    throw new Error("key is required and must be non-empty");
                if (key === "agentWallet")
                    throw new Error("'agentWallet' is a reserved key and cannot be set via set_metadata");
