 ...
 */
export async function fetchTokenBalance(walletAddress, tokenAddress, provider) {
...
async function fetchBalancesViaBatchRpc(walletAddress, tokens, tokenAddresses, provider) {
    const results = await Promise.all(tokenAddresses.map((addr) => fetchTokenBalance(walletAddress, addr, provider)));
...
async function fetchBalancesViaBalanceChecker(walletAddress, tokens, tokenAddresses, provider) {
    ...
    const tokenService = getTokenService();
    const decimalsResults = await Promise.all(tokenAddresses.map((addr) => tokenService.getDecimalsAsync(addr)));
    return tokens.map((token, i) => ({
        token,
        tokenAddress: tokenAddresses[i],
 ...
 */
export async function fetchTokenBalances(walletAddress, tokens, tokenAddresses, provider) {
    try {
        const balances = await fetchBalancesViaBalanceChecker(walletAddress, tokens, tokenAddresses, provider);
        ...
        const balances = await fetchBalancesViaBatchRpc(walletAddress, tokens, tokenAddresses, provider);
