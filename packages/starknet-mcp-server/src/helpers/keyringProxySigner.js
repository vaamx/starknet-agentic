...
import { SignerInterface, num, } from "starknet";
...
function formatProxyError(status, rawText) {
    if (!rawText) {
...
export class KeyringProxySigner extends SignerInterface {
    ...
    config;
    cachedSessionPublicKey;
    mtlsClientMaterial;
    constructor(config) {
        super();
        this.config = config;
        const hasMtlsField = Boolean(config.tlsClientCertPath || config.tlsClientKeyPath || config.tlsCaPath);
        if (hasMtlsField) {
            if (!config.tlsClientCertPath || !config.tlsClientKeyPath || !config.tlsCaPath) {
                throw new Error("Incomplete keyring mTLS client configuration; set tlsClientCertPath, tlsClientKeyPath, and tlsCaPath");
            }
    ...
    async postJsonViaMtls(url, headers, body) {
    ...
    async signMessage(_typedData, _accountAddress) {
    ...
    async signTransaction(transactions, transactionsDetail) {
        ...
        const requestPayload = {
            ...
            calls: transactions.map((call) => ({
                ...
                calldata: (call.calldata ?? []).map((value) => typeof value === "string" ? value : num.toHex(value)),
            })),
            context: {
        ...
        try {
            ...
            if (parsed.signatureMode !== "v2_snip12") {
                throw new Error("Invalid signature response from keyring proxy: signatureMode must be v2_snip12");
            }
            if (parsed.signatureKind !== "Snip12") {
                throw new Error("Invalid signature response from keyring proxy: signatureKind must be Snip12");
            }
            if (!isHexFelt(parsed.domainHash) || !isHexFelt(parsed.messageHash)) {
                throw new Error("Invalid signature response from keyring proxy: missing domainHash/messageHash");
            }
            ...
            if (!isNonEmptyString(parsed.requestId)) {
                throw new Error("Invalid signature response from keyring proxy: requestId is required");
            }
            if (parsed.requestId !== traceId) {
                throw new Error("Invalid signature response from keyring proxy: requestId does not match outbound request");
            }
            if (!parsed.audit || typeof parsed.audit !== "object") {
                throw new Error("Invalid signature response from keyring proxy: audit object is required");
            }
            if (parsed.audit.policyDecision !== "allow") {
                throw new Error("Invalid signature response from keyring proxy: audit.policyDecision must be allow");
            }
            if (!isRfc3339Timestamp(parsed.audit.decidedAt)) {
                throw new Error("Invalid signature response from keyring proxy: audit.decidedAt must be an RFC3339 timestamp");
            }
            if (!isNonEmptyString(parsed.audit.keyId)) {
                throw new Error("Invalid signature response from keyring proxy: audit.keyId is required");
            }
            if (this.config.keyId && parsed.audit.keyId !== this.config.keyId) {
                throw new Error("Invalid signature response from keyring proxy: audit.keyId does not match requested keyId");
            }
            if (!isNonEmptyString(parsed.audit.traceId)) {
                throw new Error("Invalid signature response from keyring proxy: audit.traceId is required");
            }
            if (parsed.audit.traceId !== traceId) {
                throw new Error("Invalid signature response from keyring proxy: audit.traceId does not match outbound traceId");
            }
            const allowedSignerProviders = ["local", "dfns"];
            if (!allowedSignerProviders.includes(parsed.signerProvider)) {
                throw new Error("Invalid signature response from keyring proxy: signerProvider must be local or dfns");
            }
            if (!isHexFelt(parsed.sessionPublicKey)) {
                throw new Error("Invalid signature response from keyring proxy: sessionPublicKey is required");
            }
            if (!Array.isArray(parsed.signature) || parsed.signature.length !== 4) {
                throw new Error("Invalid signature response from keyring proxy: expected [pubkey, r, s, valid_until]");
            }
            ...
            if (!feltEqualsHex(parsed.sessionPublicKey, signaturePubKey)) {
                throw new Error("Invalid signature response from keyring proxy: sessionPublicKey does not match signature pubkey");
            }
            if (!feltEqualsHex(signatureValidUntil, requestedValidUntilHex)) {
                throw new Error("Invalid signature response from keyring proxy: signature valid_until does not match requested window");
            }
            if (this.cachedSessionPublicKey &&
                !feltEqualsHex(this.cachedSessionPublicKey, resolvedSessionPublicKey)) {
                throw new Error("Invalid signature response from keyring proxy: session public key changed unexpectedly");
            }
