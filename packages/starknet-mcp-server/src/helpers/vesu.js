...
export const VESU_PRIME_POOL = "0x0451fe483d5921a2919ddd81d0de6696669bccdacd859f72a4fba7656b97c3b5";
/**
 * Fetch vToken address for a given pool and asset from Vesu PoolFactory.
 */
export async function getVTokenAddress(provider, poolAddress, assetAddress) {
 ...
 */
export function buildDepositCalls(assetAddress, vTokenAddress, amountWei, receiver) {
    if (amountWei <= 0n) {
 ...
 */
export function buildWithdrawCalls(vTokenAddress, amountWei, receiver, owner) {
    if (amountWei <= 0n) {
        throw new Error("amount must be positive");
    }
    const parsedVToken = validateAndParseAddress(vTokenAddress);
