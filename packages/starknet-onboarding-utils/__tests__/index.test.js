...
import { deployAccountViaFactory, formatBalance, parseFactoryAccountDeployedEvent, waitForTransactionWithTimeout, } from "../src/index.js";
describe("@starknet-agentic/onboarding-utils", () => {
    describe("waitForTransactionWithTimeout", () => {
        ...
        it("rejects with a clear error when timeout is exceeded", async () => {
            ...
            try {
                ...
                expect(String(err?.message || err)).toContain("waitForTransaction timed out");
                expect(String(err?.message || err)).toContain("0xdead");
            }
            finally {
                vi.useRealTimers();
            }
    ...
    describe("parseFactoryAccountDeployedEvent", () => {
        it("returns nulls when receipt has no events", () => {
            expect(parseFactoryAccountDeployedEvent({ factoryAddress: "0x1", receipt: {} })).toEqual({ accountAddress: null, agentId: null });
        });
    ...
    describe("deployAccountViaFactory", () => {
        it("calls account.execute in non-gasfree mode and parses event", async () => {
            ...
            try {
                ...
                const firstCall = executeCalls[0];
        ...
        it("calls account.executePaymasterTransaction in gasfree mode", async () => {
            ...
            expect(paymasterDetails?.feeMode?.mode).toBe("sponsored");
        });
