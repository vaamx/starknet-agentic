...
- **Scarb 2.14.x** - Cairo package manager
- **snforge 0.54.x** - Starknet Foundry testing framework
...
<Callout type="tip" title="Recommended: Install via asdf">
  ...
  asdf set --home starknet-foundry 0.54.0
  ```
...
```bash
scarb --version  # Should show 2.14.x
snforge --version  # Should show 0.54.x
```
...
- `IdentityRegistry.contract_class.json`
- `ReputationRegistry.contract_class.json`
- `ValidationRegistry.contract_class.json`
...
```bash
cd contracts/erc8004-cairo/e2e-tests
...
These addresses are a convenience snapshot. For the canonical verified status (including historical Sepolia set, class hashes, and first-seen blocks), see `docs/DEPLOYMENT_TRUTH_SHEET.md` in the repo.
</Callout>
...
<Callout type="warning" title="Upgradeable By Owner">
The current ERC-8004 registries are upgradeable via Starknet class replacement (`replace_class`), gated by owner-only `upgrade()`. Treat owner key management and upgrade procedures as production-critical controls.
</Callout>

Operational guidance:

1. Use multisig/governance as owner (not a single EOA).
2. Audit and verify class hashes before calling `upgrade()`.
3. Monitor `Upgraded` events and maintain rollback playbooks.
...
| `Build fails` | Ensure Scarb 2.14.x is installed. Check `Scarb.toml` dependencies. |

### Getting Help

- [Starknet Discord](https://discord.gg/starknet)
- [Starknet Foundry Docs](https://foundry-rs.github.io/starknet-foundry/)
- [Starknet Agentic Issues](https://github.com/keep-starknet-strange/starknet-agentic/issues)
