...
This page documents all 10 tools exposed by the Starknet MCP Server (9 core + 1 conditional). Each tool includes its description, parameters, return values, and usage examples.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `public_key` | string | Yes | Stark public key (felt, 0x-prefixed) for the new account |
| `token_uri` | string | Yes | Token URI for agent identity metadata (IPFS, Arweave, etc.) |
| `salt` | string | No | Deploy salt (felt). Random if omitted for deterministic addresses |
