...
exports.DOC_CATEGORIES = void 0;
exports.getAllDocPages = getAllDocPages;
exports.findDocPage = findDocPage;
exports.getAdjacentPages = getAdjacentPages;
...
function getAllDocPages() {
    return exports.DOC_CATEGORIES.flatMap((category) => category.pages.map((page) => ({
        ...page,
...
function findDocPage(categorySlug, pageSlug) {
    const category = exports.DOC_CATEGORIES.find((c) => c.slug === categorySlug);
    if (!category)
        return null;
...
function getAdjacentPages(categorySlug, pageSlug) {
    const allPages = getAllDocPages();
    const currentIndex = allPages.findIndex((p) => p.categorySlug === categorySlug && p.slug === pageSlug);
    if (currentIndex === -1) {
