...
async function CodeBlock({ code, language = "typescript", filename, showLineNumbers = false, showCopyButton = true, }) {
    ...
    if (lang === "text") {
        ...
        {filename && (<div className="bg-[#161b22] px-4 py-2 text-sm text-gray-400 border-b border-gray-700 font-mono flex items-center justify-between rounded-t-lg border-2 border-b-0 border-black">
            <span>{filename}</span>
          </div>)}
        {showCopyButton && <CopyButton_1.CopyButton code={trimmedCode}/>}
        <pre className={`!bg-[#0d1117] !m-0 p-4 overflow-x-auto text-sm leading-relaxed text-gray-100 whitespace-pre border-2 border-black shadow-neo ${filename ? 'rounded-t-none' : 'rounded-lg'}`} style={{ fontFamily: 'var(--font-jetbrains-mono), "JetBrains Mono", ui-monospace, SFMono-Regular, "SF Mono", Menlo, Monaco, Consolas, monospace' }}>
          {trimmedCode}
        </pre>
      </div>);
    }
    const html = await (0, shiki_1.codeToHtml)(trimmedCode, {
        lang: lang,
