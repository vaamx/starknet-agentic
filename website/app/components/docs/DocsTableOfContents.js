...
exports.DocsTableOfContents = DocsTableOfContents;
exports.extractHeadings = extractHeadings;
exports.extractHeadingsFromDOM = extractHeadingsFromDOM;
const react_1 = require("react");
function DocsTableOfContents({ items }) {
    ...
    return (<nav className="space-y-1" aria-label="Table of contents">
        ...
        {validItems.map((item) => {
            ...
            return (<li key={item.id}>
              <a href={`#${item.id}`} onClick={(e) => {
                ...
                }} className={`block py-1 text-sm transition-colors ${item.level === 2 ? "pl-3" : "pl-6"} ${isActive
                    ? "text-neo-purple font-medium border-l-2 border-neo-purple -ml-[2px]"
                    : "text-neo-dark/60 hover:text-neo-dark"}`} aria-current={isActive ? "location" : undefined}>
                {item.title}
              </a>
            </li>);
        })}
