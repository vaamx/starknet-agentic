...
async function fetchViaBalanceChecker(provider, walletAddress, tokenInfos) {
...
async function fetchViaBatchRpc(provider, walletAddress, tokenInfos) {
    const balanceResults = await Promise.all(tokenInfos.map(async (info) => {
